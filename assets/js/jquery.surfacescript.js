...
(function($) {
	...
	var SurfaceSlider = function(target, settings) {
		...
		var SLIDER 	 = $(target);
		var SURFACE 	 = 'div.surfaceslider';
		var SLIDES 	 = 'ul.ss-slides';
		var SLIDE  	 = 'li.ss-slide';
		var ELEMENTS = '> *';
		...
		// The slider constructor: runs automatically only the first time, sets the basic needs of the slider and the preloader then runs Surface Slider
		function init() {			
			// Add wrappers and classes
			SLIDER.wrapInner('<div class="surfaceslider" />');
			SLIDER.find(SURFACE + ' > ul').addClass('ss-slides');
			SLIDER.find(SURFACE + ' ' + SLIDES + ' > li').addClass('ss-slide');
			...
			if(total_slides == 1) {
				...
				var prepend = SLIDER.find(SURFACE).find(SLIDES);
			...
			if(settings.showControls) {
				SLIDER.find(SURFACE).append('<div class="ss-controls"><span class="ss-next"></span><span class="ss-previous"></span></div>');
			}
			
			// Show navigation
			if(settings.showNavigation) {
				var nav = '<div class="ss-navigation">';
				for(var i = 0; i < total_slides; i++) {
					nav += '<span class="ss-slide-link"></span>';
				}
				nav += '</div>';
				SLIDER.find(SURFACE).append(nav);
			}
			
			// Show progress bar
			if(settings.showProgressBar) {
				SLIDER.find(SURFACE).append('<div class="ss-progress-bar"></div>');
			}
			else {
				SLIDER.find(SURFACE).append('<div class="ss-progress-bar ss-progress-bar-hidden"></div>');
			}
			...
			if(typeof surfaceslider_is_wordpress_admin == 'undefined' || ! surfaceslider_is_wordpress_admin) {
				loadWindow();
			}
			else {
				loadedWindow();
		...
		function addListeners() {
			...
			SLIDER.find(SURFACE).find('.ss-controls > .ss-previous').click(function() {
				changeSlide(getPreviousSlide());
			});
			
			// Next Control click
			SLIDER.find(SURFACE).find('.ss-controls > .ss-next').click(function() {
				changeSlide(getNextSlide());
			});
			
			// Swipe and drag
			if(settings.enableSwipe) {
				SLIDER.find(SURFACE).surfaceslider_swipe({
			...
			SLIDER.find(SURFACE).find('.ss-navigation > .ss-slide-link').click(function() {
			...
			if(settings.pauseOnHover) {
				SLIDER.find(SURFACE).find(SLIDES).hover(function() {
					pause();
				});
				
				SLIDER.find(SURFACE).find(SLIDES).mouseleave(function() {
					resume();
				});
			}
		}
		...
		function setPreloader() {
			SLIDER.find(SURFACE).find(SLIDES).css('display', 'none');
			SLIDER.find(SURFACE).find('.ss-progress-bar').css('display', 'none');
			SLIDER.find(SURFACE).find('.ss-navigation').css('display', 'none');
			SLIDER.find(SURFACE).find('.ss-controls').css('display', 'none');
			SLIDER.find(SURFACE).append('<div class="ss-preloader"><div class="ss-loader"></div></div>');
		}
		
		// Removes gif loader
		function unsetPreloader() {
			SLIDER.find(SURFACE).find(SLIDES).css('display', 'block');
			SLIDER.find(SURFACE).find('.ss-progress-bar').css('display', 'block');
			SLIDER.find(SURFACE).find('.ss-navigation').css('display', 'block');
			SLIDER.find(SURFACE).find('.ss-controls').css('display', 'block');
			SLIDER.find(SURFACE).find('.ss-preloader').remove();
		}
		...
		// Runs Surface Slider from the current slide
		function play() {
		...
		function pause() {
			if(! paused && can_pause) {
				...
				var progress_bar = SLIDER.find(SURFACE).find('.ss-progress-bar');
		...
		// Returns the Surface Slider container width
		function getWidth() {
			return SLIDER.find(SURFACE).width();
		}
		
		// Returns the Surface Slider container height
		function getHeight() {
			return SLIDER.find(SURFACE).height();
		}
		...
		function getItemData(item, data) {
			...
			if(item.parent('ul').hasClass('ss-slides')) {
				is_slide = true;
			}
			else {
				is_slide = false;
		...
		function getSlides() {
			return SLIDER.find(SURFACE).find(SLIDES).find(SLIDE);
		}
		...
		function setNavigationLink() {
			var nav = SLIDER.find(SURFACE).find('.ss-navigation');
			var links = nav.find('> .ss-slide-link');
			
			links.each(function() {
				var link = $(this);
				
				if(link.index() == current_slide) {
					link.addClass('ss-active');
				}
				else {
					link.removeClass('ss-active');
				}
			});
		}
	...
	/**************************/
	/** SURFACE SLIDER PLUGIN **/
	/**************************/
	
	// Plugin
	$.fn.surfaceSlider = function(options) {	
        var settings = $.extend({
			layout					: 'fixed',
			responsive				: true,
			startWidth				: 1170,
        ...
        return this.each(function() {
			new SurfaceSlider(this, settings);
        });
    };
