...
// Code output
function surfaceSlider($alias) {
	SurfaceSliderFrontend::output($alias, true);
}

function getSurfaceSlider($alias) {
	SurfaceSliderFrontend::output($alias, false);
}

class SurfaceSliderFrontend {
	
	public static function setNotAdminJs() {
		add_action('wp_enqueue_scripts', 'SurfaceSliderFrontend::notAdminJs');
	}
	...
	public static function shortcode($atts) {
		...
		if(! $a['alias']) {
			return __('You have to insert a valid alias in the shortcode', 'surfaceslider');
		}
		else {
			return SurfaceSliderFrontend::output($a['alias'], false);
		}
	}

	public static function addShortcode() {
		add_shortcode('surfaceslider', array( __CLASS__, 'shortcode'));
	}

	public static function output($alias, $echo) {
		global $wpdb;
		
		$slider = $wpdb->get_row('SELECT * FROM ' . $wpdb->prefix . 'surfaceslider_sliders WHERE alias = \'' . $alias . '\'');
		
		if(! $slider) {
			if($echo) {
				_e('The slider hasn\'t been found', 'surfaceslider');
				return;
			}
			else {
				return __('The slider hasn\'t been found', 'surfaceslider');
			}
		}
		
		$slider_id = $slider->id;
		$slides = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'surfaceslider_slides WHERE slider_parent = ' . $slider_id . ' ORDER BY position');
		
		$output = '';
		
		$output .= '<div style="display: none;" class="surfaceslider-slider surfaceslider-slider-' . $slider->layout . ' surfaceslider-slider-' . $alias . '" id="surfaceslider-' . $slider_id . '">' . "\n";
		$output .= '<ul>' . "\n";
		foreach($slides as $slide) {
			...
			$elements = $wpdb->get_results('SELECT * FROM ' . $wpdb->prefix . 'surfaceslider_elements WHERE slider_parent = ' . $slider_id . ' AND slide_parent = ' . $slide_parent);	
		...
		$output .= '$("#surfaceslider-' . $slider_id  . '").surfaceSlider({' . "\n";
